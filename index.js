...
async function run() {
  try {
    ...
    const bookingCollaction = client
      .db("tour-management")
    ...
    app.post("/my-bookings", async (req, res) => {
      try {
        const booking = req.body; // <--- data from frontend
        const result = await bookingCollaction.insertOne(booking); // save in DB
        res.send(result);
      } catch (error) {
        console.error(error);
    ...
    app.put("/update-ture/:id", async (req, res) => {
      ...
      const result = await turesCollaction.updateOne(
        filter,
        updateDoc,
        options
      );
